...
const { makeFoldersArray, makeNewFolder, makeUpdatedFolder, makeShamFolder } = require('./folders-fixtures');
...
describe('Folders Endpoints', function() {
    ...
    describe(`POST /api/folders`, () => {
        it(`responds with 200 and the created folder`, () => {
            ...
            return supertest(app)
             ...
             .send(testFolder)
             .expect(201)
             .expect(res => { expect(res.body.name).to.eql(testFolder.name)} );
        });
    });
    ...
    describe(`DELETE /api/folders`, () => {
        ...
        context(`Given folders in the database`, () => {
            ...
            before('insert folders', () => { //change to beforeeach if other tests are added 
                return db
                 .into('folders')
                 .insert(testFolders);
            });
            ...
            it(`responds with 200 when folder in database is deleted`, () => {
                ...
                const currentFolders = testFolders.filter(folder => {folder.folder_id !== deletefolder_id});
                ...
                return supertest(app)
                 ...
                 .then(() => {
                     supertest(app)
                      ...
                      .expect(201)
                      .expect(res => { expect(res.name).to.eql(updatedFolder.name) })
                 });
            });
        });
    });
